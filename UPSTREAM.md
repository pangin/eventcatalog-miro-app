# UPSTREAM — eventcatalog-miro-app

이 저장소는 `event-catalog/eventcatalog-miro-app`의 **GitHub fork**(fork network 내부, public)입니다. 원본 기준선과 내부 수정선을 분리해 추적하며, 기계가 읽는 값은 [upstream.json](./upstream.json)에 있습니다.

| 항목 | 값 |
|---|---|
| upstream | https://github.com/event-catalog/eventcatalog-miro-app (default branch `main`) |
| fork | https://github.com/pangin/eventcatalog-miro-app |
| 관계 | GitHub fork (public). 원본 fork network에 연결되어 있어 GitHub compare/PR UI로 upstream과 비교 가능 |
| 기준(pinned) SHA | `01a17a3541f20328682ba728c5b11ead506f8036` (2026-04-07T12:59:35+01:00) |
| 현재 fetched / integrated SHA | `upstream.json` → `.sha.fetched`, `.sha.integrated` (sync workflow가 갱신) |
| owner | 성욱 (pangin) |
| 역할 분류 | **optional** — Miro Web SDK 앱. EventCatalog export JSON을 사용자가 직접 가져와 사용하며 core에 의존하지 않는다. |
| 라이선스(root) | MIT |
...
```bash
git clone https://github.com/pangin/eventcatalog-miro-app.git && cd eventcatalog-miro-app
git remote add upstream https://github.com/event-catalog/eventcatalog-miro-app.git
...
|---|---|
| Node | 20.x |
| package manager | npm (lockfile: package-lock.json, pnpm-lock.yaml) |
| install | `npm ci` |
| build | `npm run build` |
| test | `n/a (테스트 스크립트 없음)` |
| run | `npm start  # vite dev 서버 http://localhost:3000 (Miro 보드 안에서만 기능)` |
| smoke | npm run build 후 dist/index.html, dist/app.html 존재 |
